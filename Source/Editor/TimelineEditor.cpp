...
TimelineEditor::TimelineEditor() : Editor("Timeline Editor") {}

void TimelineEditor::Draw(Application& app) {
    Profile;
    ...
    if (ImGui::Begin(name_.c_str(), &isVisible_, ImGuiWindowFlags_NoCollapse)) {
        ...
        if (ImGui::IsItemActive()) {
            if (!isDraggingSplitter_) {
                ...
                leftPanelWidth_ = std::max(200.0f, std::min(leftPanelWidth_, ImGui::GetWindowWidth() - 200.0f));
            }
        } else {
...
void TimelineEditor::DrawTreeView() {
    if (selectedTimeline_) {
        if (ImGui::Button("Play")) selectedTimeline_->Play();
        ImGui::SameLine();
        if (ImGui::Button("Pause")) selectedTimeline_->Pause();
        ImGui::SameLine();
        if (ImGui::Button("Stop")) selectedTimeline_->Stop();
        ImGui::SameLine();
        ImGui::TextDisabled("|");
        ImGui::SameLine();
    ...
    for (auto& timeline : timelines) {
        ...
        if (ImGui::BeginPopupContextItem()) {
            if (ImGui::MenuItem("Add Track")) {
                ...
                for (Entity entity : livingEntities) {
                    ...
                    if (name.empty()) name = "Entity_" + std::to_string(entity);
        ...
        if (nodeOpen) {
            ...
            for (auto& track : tracks) {
                ...
                if (!trackLabel.empty()) trackLabel += " ";

                Entity targetEntity = track->GetTargetEntity();
                std::string entityName = currentScene_->GetWorld()->GetEntityName(targetEntity);
                if (entityName.empty()) entityName = "Entity_" + std::to_string(targetEntity);

                trackLabel += "(" + entityName + ") - " + track->GetComponentName() + "." + track->GetPropertyName();
                ...
                if (!track->IsEnabled()) ImGui::PopStyleColor();
...
void TimelineEditor::DrawRightPanel() {
    if (selectionType_ == SelectionType::CreateTimeline) {
        ...
        if (ImGui::Button("Cancel##CancelCreateTimeline")) {
            selectionType_ = isEditing ? SelectionType::Timeline : SelectionType::None;
            if (!isEditing) selectedTimeline_ = nullptr;
        }
        ...
        if (!availableEntities_.empty()) {
            if (ImGui::BeginCombo("Entity", availableEntities_[selectedEntityIndex_].c_str())) {
                for (size_t i = 0; i < availableEntities_.size(); ++i) {
                    bool isSelected = (selectedEntityIndex_ == (int)i);
                    if (ImGui::Selectable(availableEntities_[i].c_str(), isSelected)) {
                        selectedEntityIndex_ = (int)i;
                    }
        ...
        if (ImGui::BeginCombo("Component", availableComponents_[selectedComponentIndex_].c_str())) {
            for (size_t i = 0; i < availableComponents_.size(); ++i) {
                bool isSelected = (selectedComponentIndex_ == (int)i);
                if (ImGui::Selectable(availableComponents_[i].c_str(), isSelected)) {
                    selectedComponentIndex_ = (int)i;
                }
                if (isSelected) ImGui::SetItemDefaultFocus();
            }
            ImGui::EndCombo();
        ...
        if (!availableProperties_.empty()) {
            if (ImGui::BeginCombo("Property", availableProperties_[selectedPropertyIndex_].c_str())) {
                for (size_t i = 0; i < availableProperties_.size(); ++i) {
                    bool isSelected = (selectedPropertyIndex_ == (int)i);
                    if (ImGui::Selectable(availableProperties_[i].c_str(), isSelected)) {
                        selectedPropertyIndex_ = (int)i;
                    }
                    if (isSelected) ImGui::SetItemDefaultFocus();
                }
                ImGui::EndCombo();
        ...
        if (ImGui::Button("Create##CreateTrackBtn")) {
            if (!availableEntities_.empty()) {
                ...
                if (currentScene_->GetWorld()->GetEntityByName(entityName, &entity)) {
                    auto* newTrack = selectedTimeline_->AddTrack<float>(trackNameBuffer_, entity, componentName, propertyName);
...
void TimelineEditor::DrawTimelineSettings() {
...
void TimelineEditor::DrawPlaybackControls() {
    if (!selectedTimeline_) return;

    if (ImGui::Button("Play##PlayBtn")) selectedTimeline_->Play();
    ImGui::SameLine();
    if (ImGui::Button("Pause##PauseBtn")) selectedTimeline_->Pause();
    ImGui::SameLine();
    if (ImGui::Button("Stop##StopBtn")) selectedTimeline_->Stop();
...
void TimelineEditor::DrawGraphicalKeyframeEditor() {
    ...
    if (ImGui::Button("Auto-fit Range")) {
        auto& keyframes = floatTrack->GetKeyframes();
        if (!keyframes.empty()) {
            graphMinValue_ = graphMaxValue_ = keyframes[0].value;
    ...
    ImGui::Separator();
    ImGui::TextDisabled("Left-click: Add/Select | Right-click: Delete | Drag: Move");
    ImGui::Separator();

    ImVec2 graphSize = ImGui::GetContentRegionAvail();
    graphSize.y = std::max(graphSize.y - 50.0f, 200.0f);
...
void TimelineEditor::DrawGraph(Track<float>* track, const ImVec2& graphMin, const ImVec2& graphSize) {
    ...
    for (size_t i = 0; i < keyframes.size(); ++i) {
        ...
        ImU32 color = (i == (size_t)selectedKeyframeIndex_) ? IM_COL32(255, 255, 0, 255) : IM_COL32(255, 100, 100, 255);
        drawList->AddCircleFilled(ImVec2(x, y), 5.0f, color);
        drawList->AddCircle(ImVec2(x, y), 5.0f, IM_COL32(255, 255, 255, 255), 12, 1.5f);
    }
}

bool TimelineEditor::HandleGraphInput(Track<float>* track, const ImVec2& graphMin, const ImVec2& graphSize) {
    ImVec2 graphMax = ImVec2(graphMin.x + graphSize.x, graphMin.y + graphSize.y);
    ...
    if (!inGraph && !isDraggingKeyframe_) return false;
    ...
    if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && inGraph) {
        ...
        for (size_t i = 0; i < keyframes.size(); ++i) {
            ...
            if (dist < 8.0f) {
                selectedKeyframeIndex_ = (int)i;
                draggingKeyframeIndex_ = (int)i;
    ...
    if (isDraggingKeyframe_ && ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
        if (draggingKeyframeIndex_ >= 0 && draggingKeyframeIndex_ < (int)keyframes.size()) {
    ...
    if (ImGui::IsMouseClicked(ImGuiMouseButton_Right) && inGraph) {
        for (size_t i = 0; i < keyframes.size(); ++i) {
            ...
            if (dist < 8.0f) {
                ...
                if (selectedKeyframeIndex_ == (int)i) selectedKeyframeIndex_ = -1;
                break;
            }
        }
    }
...
int TimelineEditor::CountKeyframesOutsideDuration(float duration) {
